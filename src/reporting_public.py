...
def _open_portfolio_return(portfolio: pd.DataFrame) -> float | None:
    """Return of the current model portfolio since each position's entry.

    Open-position returns already include the purchase cost. Target weights are
    kept as-is, so uninvested cash contributes a zero return.
    """
    if portfolio.empty or not {"target_weight", "open_return"}.issubset(portfolio.columns):
        return None
    weights = pd.to_numeric(portfolio["target_weight"], errors="coerce")
    returns = pd.to_numeric(portfolio["open_return"], errors="coerce")
    if weights.isna().any() or returns.isna().any():
        return None
...
def build_public_report(
    ...
    sigma_open_return = _open_portfolio_return(sigma)
    delta_open_return = _open_portfolio_return(delta)
    ...
    <section><h2>Resumen ejecutivo</h2><div class="cards"><div class="card"><strong>Sigma-6</strong><div class="big">{pct(sigma_open_return)}</div><div class="muted">Rentabilidad ponderada de las posiciones abiertas desde su ingreso</div><p>{len(sigma)} posiciones · {pct(sigma_cash)} en caja</p></div><div class="card"><strong>Delta-12</strong><div class="big">{pct(delta_open_return)}</div><div class="muted">Rentabilidad ponderada de las posiciones abiertas desde su ingreso</div><p>{len(delta)} posiciones · {pct(delta_cash)} en caja</p></div></div></section>
